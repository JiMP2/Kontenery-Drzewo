...
public:
    ...
    virtual unsigned int size(void) const = 0;
     ...
     //\return true if operation successful.
    virtual bool remove(unsigned int _position) = 0;
    ...
    int indexOf(T const& _value, unsigned int _from = 0) const;
    ...
    bool contains(T const& _value, unsigned int _from = 0) const;
    ...
    T& operator[](unsigned int n) const;
     ...
     //\brief Overload of operator=.
     //\_right Operand of overloaded operator.
    virtual aghContainer<T>& operator=(aghContainer<T> const& _right);
    ...
    template <typename T>
    int aghContainer<T>::indexOf(T const& _value, unsigned int _from) const
    {
          for(unsigned int i = _from; i<size(); i++)
          {
               if(this->at(i) == _value){
                    return i;
               }
    ...
    template <typename T>         
    bool aghContainer<T>::contains(T const& _value, unsigned int _from) const
    {
    ...
    template <typename T>
    T& aghContainer<T>::operator[](unsigned int n) const
    {
        return at(n);
    }
